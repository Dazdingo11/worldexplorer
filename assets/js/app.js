...
async function fetchNewsForCountry(country) {
    ...
    if (common || capital) {
        ...
        if (q) {
            ...
            if (
                !r3.error &&
                Array.isArray(r3.articles) &&
                r3.articles.length > 0
            )
                return r3;
        }
    }
...
async function renderCitiesPanel(country, neighborsFull = []) {
    ...
    <p><strong>Coordinates:</strong> ${
        capCoords
            ? `${capCoords[0].toFixed(2)}, ${capCoords[1].toFixed(2)}`
            : "—"
    }</p>
  ...
  `;
    ...
    if (
        ...
        const nearby = neighborsFull
            .map((n) => {
                const nCap = Array.isArray(n?.capital)
                    ? n.capital[0]
                    : n?.capital;
        ...
        if (nearby.length) {
            ...
            nearby.forEach((x) => {
                ...
                li.textContent = `${x.cap} (${x.name}) – ${Math.round(
                    x.dist
                )} km`;
                ul.appendChild(li);
            });
...
const insertClasses = () => {
    const countryPanelWrapper = document.querySelector(
        ".country-panel-wrapper"
    );
